...
# delete-service public-staging fs-intake-api-staging login-service-provider
# delete-service public-production fs-intake-api login-service-provider
createIntakeServices public-production "${MIDDLE_SERVICE_PROD_MIDDLELAYER_BASE_URL}" "${MIDDLE_SERVICE_PROD_MIDDLELAYER_PASSWORD}" "${MIDDLE_SERVICE_PROD_MIDDLELAYER_USERNAME}" "${INTAKE_CLIENT_SERVICE_PROD_INTAKE_CLIENT_BASE_URL}" "${INTAKE_CLIENT_SERVICE_PROD_INTAKE_PASSWORD}" "${INTAKE_CLIENT_SERVICE_PROD_INTAKE_USERNAME}" "${LOGIN_SERVICE_PROVIDER_PROD_cert}" "${LOGIN_SERVICE_PROVIDER_PROD_issuer}" "${LOGIN_SERVICE_PROVIDER_PROD_private}" "${LOGIN_SERVICE_PROVIDER_PROD_decryptCert}"
createIntakeServices public-staging "${MIDDLE_SERVICE_DEV_MIDDLELAYER_BASE_URL}" "${MIDDLE_SERVICE_DEV_MIDDLELAYER_PASSWORD}" "${MIDDLE_SERVICE_DEV_MIDDLELAYER_USERNAME}" "${INTAKE_CLIENT_SERVICE_DEV_INTAKE_CLIENT_BASE_URL}" "${INTAKE_CLIENT_SERVICE_DEV_INTAKE_PASSWORD}" "${INTAKE_CLIENT_SERVICE_DEV_INTAKE_USERNAME}" "${LOGIN_SERVICE_PROVIDER_DEV_cert}" "${LOGIN_SERVICE_PROVIDER_DEV_issuer}" "${LOGIN_SERVICE_PROVIDER_DEV_private}" "${LOGIN_SERVICE_PROVIDER_DEV_decryptCert}"
